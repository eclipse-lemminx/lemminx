...
public class XMLElementGenerator {
	 ...
	 *                           otherwise.
	 */
	public XMLElementGenerator(SharedSettings sharedSettings, String whitespacesIndent, String lineDelimiter,
	...
	public XMLElementGenerator(SharedSettings sharedSettings, boolean autoCloseTags, String whitespacesIndent,
	...
	/**
	 * Sets namespace prefix mappings for document generation. When set, child
	 * elements whose namespace differs from the root namespace will be generated
	 * with the corresponding prefix.
	 *
	 * @param rootNamespace      the root element's namespace (default namespace).
	 * @param namespacePrefixes  mapping from namespace URI to prefix.
	 */
	public void setNamespacePrefixes(String rootNamespace, Map<String, String> namespacePrefixes) {
		this.rootNamespace = rootNamespace;
		this.namespacePrefixes = namespacePrefixes;
	}

	public void setUseTypeDefaults(boolean useTypeDefaults) {
		this.useTypeDefaults = useTypeDefaults;
	}
	...
	private String computeElementPrefix(CMElementDeclaration element) {
		...
		if (ns == null || ns.equals(rootNamespace)) {
			return null;
		}
		return namespacePrefixes.get(ns);
	}
	...
	private int generate(CMElementDeclaration elementDeclaration, String prefix, boolean generateEndTag,
		...
		Collection<CMElementDeclaration> children = generateOnlyRequired ? elementDeclaration.getRequiredElements()
				: elementDeclaration.getContentElements();
		if (generateOnlyChildren) {
			for (CMElementDeclaration child : children) {
				...
				String childPrefix = computeElementPrefix(child);
				if (childPrefix == null) childPrefix = prefix;
				snippetIndex = generate(child, childPrefix, true, false, level + 1, snippetIndex, xml, generatedElements,
						generateOnlyRequired);
		...
		if (children.size() > 0) {
			...
			if ((level < maxLevel)) {
				for (CMElementDeclaration child : children) {
					...
					String childPrefix = computeElementPrefix(child);
					if (childPrefix == null) childPrefix = prefix;
					snippetIndex = generate(child, childPrefix, true, false, level, snippetIndex, xml, generatedElements,
							generateOnlyRequired);
					level--;
				}
				xml.linefeed();
			...
			if (!values.isEmpty()) {
				...
				String defaultValue = elementDeclaration.getDefaultValue();
				if (defaultValue == null && useTypeDefaults) {
					defaultValue = elementDeclaration.getTypeAwareDefaultValue();
				}
				if (defaultValue != null) {
					...
					if (canSupportSnippets) {
						...
						SnippetsBuilder.placeholders(snippetIndex, defaultValue, sb);
						xml.addContent(sb.toString());
					} else {
						xml.addContent(defaultValue);
					}
				}
			}
			if (canSupportSnippets) {
			...
			if (forDocumentGeneration && !hasContent) {
				xml.linefeed();
				xml.indent(level);
			}
			if (generateEndTag) {
	...
	private int generate(Collection<CMAttributeDeclaration> attributes, int level, int snippetIndex, XMLBuilder xml,
		...
		for (CMAttributeDeclaration attributeDeclaration : requiredAttributes) {
			...
			if (defaultValue == null && useTypeDefaults) {
				defaultValue = attributeDeclaration.getTypeAwareDefaultValue();
			}
	...
	private int generateXsiTypeAttribute(Collection<String> derivedTypeNames, String elementPrefix,
		...
		String defaultType = qualifiedTypes.iterator().next();
		String value = generateAttributeValue(defaultType, qualifiedTypes, canSupportSnippets, snippetIndex,
				false, sharedSettings);
	...
	public static String generateAttributeValue(String defaultValue, Collection<String> enumerationValues,
		...
		if (!canSupportSnippets) {
			if (defaultValue != null) {
				...
				value.append(enumerationValues.iterator().next());
			}
		} else {
	...
	public static MarkupContent createMarkupContent(CMElementDeclaration cmElement, ISharedSettingsRequest support) {
		String documentation = XMLElementGenerator.generateDocumentation(cmElement.getDocumentation(support),
	...
	public static MarkupContent createMarkupContent(CMAttributeDeclaration cmAttribute,
		...
		String documentation = XMLElementGenerator.generateDocumentation(cmAttribute.getAttributeNameDocumentation(request),
	...
	public static MarkupContent createMarkupContent(CMAttributeDeclaration cmAttribute, String attributeValue,
		...
		String documentation = XMLElementGenerator.generateDocumentation(
	...
	public static MarkupContent createMarkupContent(CMElementDeclaration cmElement, String textContent,
		...
		String documentation = XMLElementGenerator.generateDocumentation(cmElement.getTextDocumentation(textContent, support),
				cmElement.getDocumentURI(), support.canSupportMarkupKind(MarkupKind.MARKDOWN));
		if (documentation != null) {
			return MarkupContentFactory.createMarkupContent(documentation, MarkupKind.MARKDOWN, support);
		}
